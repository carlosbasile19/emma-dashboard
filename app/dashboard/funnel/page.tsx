...
import { fetchFunnel } from "@/lib/olivia/service";
...
export default async function FunnelPage({ searchParams }: { searchParams: SP }) {
  ...
  let result;
  try {
    result = await fetchFunnel(rangeToPeriod(range, tz));
  } catch {
    return <ErrorState copy={ERROR_COPY.funnel} />;
  }

  const f = result.data.funnel;
  const top = f.new ?? 0;
  const totalAll = Object.values(f).reduce((a, v) => a + (v ?? 0), 0);
  if (totalAll === 0) {
    return <EmptyState copy={EMPTY_COPY.funnel} />;
  }
          ...
          <div className="relative overflow-hidden rounded-[16px] bg-ink p-6 shadow-ink">
            ...
            <div className="relative">
              ...
              <div className="mt-2 text-[13.5px] leading-[1.5] text-[#B7C3C4]">
                {num(converted)} of {num(top)} leads went all the way. Every drop-off above is
                a follow-up Emma is still working.
              </div>
            </div>
          </div>
